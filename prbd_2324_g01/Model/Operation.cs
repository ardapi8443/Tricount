...
public class Operation : EntityBase<PridContext> {
    ...
    public virtual int Initiator { get; private set; }

    [Required, , ForeignKey(nameof(User))]
    public virtual int Tricount{ get; private set; }

    public virtual ICollection<Repartition> Repartitions { get; private set; } = new HashSet<Repartition>();

    public Operations(Tricount tricount, User initiator, string title, int amount) {
        Tricount = tricount;
        Initiator = initiator;
        Title = title;
        Amount = amount;
